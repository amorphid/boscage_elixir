defmodule Boscage.PapalTree do
  ...
  def insert(
        %__MODULE__{
          key: center_stored_key,
          left: nil,
          right: %__MODULE__{key: right_stored_key} = right,
          size: 2
        } = center,
        insert_key,
        insert_value
      )
      when insert_key > center_stored_key and insert_key < right_stored_key do
  ...
  def insert(
    ...
    {:ok, right2} = new() |> insert(insert_key, value)
  ...
  def insert(
        %__MODULE__{key: stored_key, left: nil, right: %__MODULE__{}, size: 2} =
          center,
        insert_key,
        value
      )
      when insert_key <= stored_key do
    {:ok, left2} = new() |> insert(insert_key, value)
    center_size = 3
